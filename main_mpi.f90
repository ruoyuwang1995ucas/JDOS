...
real (kind=8),allocatable :: w0_list(:),w_wt_arr(:,:),jdos_arr(:),jdos_arr_decay(:),jdos_sub(:)
...
allocate(jdos_arr_decay())

! set initial values of jdos_arr & jdos_arr_decay
do i=1,num_w0
  jdos_arr(i)=w0_list(i)
  jdos_arr_decay(i)=w0_list(i)
enddo

! calculate adsorption process
! initialize MPI 
...
! calculate the decay process
call MPI_INIT(ierror)
call MPI_COMM_SIZE(MPI_COMM_WORLD, proc_Tot,ierror)
call MPI_COMM_RANK(MPI_COMM_WORLD, proc_Rank, ierror) 
! scatter data to proc 0-3
call MPI_Scatter(jdos_arr_decay, sub_size, MPI_DOUBLE_PRECISION, jdos_sub, sub_size, MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierror);
! task per proc
...
call MPI_Gather(jdos_sub_decay,sub_size,MPI_DOUBLE_PRECISION, jdos_arr, sub_size, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD,ierror)
